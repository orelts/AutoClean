"""
Written by Shachar Praisler (M.Sc)
"""
import telemetry
import time
...
def periodicSend(lastSendTime, msdId):
    ...
    TX_Pixhawk_telem_per = telemetry.Telemetry(droneConnected)  # can't open SITL simulation on TX2 ARM (only on computer)
    ...
    while True:
        ...
        lat_per, lon_per, alt_per, _, height_per, _, _, _ = TX_Pixhawk_telem_per.read_information()
        ...
        if (time.time() - lastSendTime.value) >= TIME_BETWEEN_MSGS:  # no transmission was sent due to camera trigger, sending periodic message
            ...
            with msdId.get_lock():  # can be changed also inside 'main'
                ...
                print("GPS lat (MED): {}, GPS lon (MED): {}, GPS alt (MED): {}, Compass: {}, Height from home (MED): {}, Yaw: {}, Pitch: {}, Roll: {}".format(median(lat_per_med), median(lon_per_med),  # for debug
                                                                                  median(alt_per_med), heading_per, median(height_per_med), cam_yaw_per, cam_pit_per, cam_rol_per))
                
                sensors_info = (median(lat_per_med), median(lon_per_med), median(alt_per_med),
                groundspeed_per, home_location_per, battery_per,last_heartbeat_per,
                median(lat_per_med), median(lon_per_med), median(alt_per_med),
                groundspeed_per, home_location_per, battery_per,last_heartbeat_per)
                
                cursor.execute('''
                IF NOT EXISTS (SELECT * FROM  dbo.SensorsInfo WHERE ID = 1)

                    INSERT INTO dbo.SensorsInfo(ID, GPS_lat, GPS_lon, GPS_alt, groundspeed, home_location, battery, last_heartbeat)
                    VALUES(1, ?, ?, ?, ?, ?, ?, ?)

                ELSE

                    UPDATE dbo.SensorsInfo
                    SET GPS_lat = ?, GPS_lon = ?, GPS_alt = ?, groundspeed = ?, home_location = ?, battery = ?, last_heartbeat= ?
                    WHERE ID = 1
                               ''', sensors_info)
                conn.commit()
...
if __name__ == '__main__':
    droneConnected = args.is_drone; flightMode = args.is_flight; inDoor = args.in_door

    # initiate contact with flight computer #
    TX_Pixhawk_telem = telemetry.Telemetry(droneConnected)  # can't open SITL simulation on TX2 ARM (only on computer)
    ...
    while True:
        ...
        while reset_value <= 1800:  # if reset is needed, drone operator will change this channel to the value of ~2000, for 3 seconds
            ...
            try:
                lat_cam, lon_cam, alt_cam, _, height_cam, _, _, _ = TX_Pixhawk_telem.read_information()
